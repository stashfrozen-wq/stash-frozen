...
async function main() {
  ...
  // ──────────────────────────────────────────────────────────
  // STEP 7: Create customer ساكو + Invoice (5 kilo بن ساده وسط at 550)
  // ──────────────────────────────────────────────────────────
  console.log('📝 STEP 7: Creating customer ساكو + invoice...\n');

  // 7a. Find or create customer
  const customerPhone = '+201030766947';
  const customerName = 'ساكو';
  ...
  if (!customer) {
    customer = await prisma.customer.create({
      data: {
        name: customerName,
        phone: customerPhone,
        address: 'امام مطعم الكيلاني - المركزيه',
      },
    });
    console.log(`   👤 Created customer: "${customerName}" (${customerPhone})`);
  } else {
    console.log(`   👤 Found customer: "${customerName}"`);
  }

  // 7b. Find the product (بن ساده وسط)
  const bunSadaWasat = await prisma.product.findFirst({
    where: { name: 'ساده وسط', categoryId: catBun.id },
  });

  if (!bunSadaWasat) {
    console.log('   ❌ ERROR: Could not find product "ساده وسط" — skipping invoice');
  } else {
    // 7c. Check inventory
    const location = await prisma.location.findFirst();
    if (!location) {
      console.log('   ❌ ERROR: No location found — skipping invoice');
    } else {
      const inv = await prisma.inventory.findFirst({
        where: { productId: bunSadaWasat.id, locationId: location.id },
      });
      ...
      if (available < orderQty) {
        console.log(`   ⚠️  WARNING: Only ${available} كيلوا available, need ${orderQty}. Proceeding anyway...`);
      }

      // 7d. Create invoice in a transaction
      await prisma.$transaction(async (tx) => {
        ...
        const invoice = await tx.invoice.create({
          data: {
            ...
            customerName,
            customerPhone,
            customerAddress: 'امام مطعم الكيلاني - المركزيه',
        ...
        await tx.transaction.create({
          data: {
            ...
            notes: `بيع ${orderQty} كيلوا بن ساده وسط لساكو - محمد/فارس`,
            items: {
              create: [
                {
                  productId: bunSadaWasat.id,
        ...
        console.log(`      Contact: محمد - فارس`);
        console.log(`      Address: امام مطعم الكيلاني - المركزيه`);
        console.log(`      Phone: ${customerPhone}`);
        console.log(`      Items: ${orderQty} كيلوا بن ساده وسط × ${unitPrice} = ${totalAmount} EGP`);
        console.log(`      Payment: CASH`);
        console.log(`      Inventory after sale: ${available - orderQty} كيلوا`);
      });
    }
  }
